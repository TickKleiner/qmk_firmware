...
#include "keyboards/ergohaven/src/info.h"
#include "keyboards/ergohaven/src/hid/hid.h"
...
oled_mode_t get_oled_mode_on_half(bool on_master) {
    if (on_master) return oled_config.master;
    // first two modes swapped for slave
    if (oled_config.slave == OLED_STATUS_CLASSIC) return OLED_SPLASH;
    if (oled_config.slave == OLED_SPLASH) return OLED_STATUS_CLASSIC;
    return oled_config.slave;
}
oled_mode_t get_oled_mode(void) {
    return get_oled_mode_on_half(is_keyboard_master());
}
uint8_t split_get_lang(void) {
    return is_keyboard_master() ? 0 : oled_config.lang;
}
bool split_get_mac(void) {
    return is_keyboard_master() ? false : oled_config.mac;
}
bool split_get_caps_word(void) {
    return is_keyboard_master() ? is_caps_word_on() : oled_config.caps_word;
}
...
void render_status_minimalistic(void) {
    ...
    oled_set_cursor(0, 3);
    oled_write(split_get_lang() == 0 ? "     " : " RU  ", false);
...
void via_set_layout_options_kb(uint32_t value) {
    if (oled_config.raw == value) return;
    oled_config.raw          = value;
...
void housekeeping_task_split_oled(void) {
    if (is_keyboard_master()) {
        ...
        if (timer_elapsed32(last_sync) > 500) {
            oled_config.lang      = split_get_lang();
            oled_config.mac       = split_get_mac();
            oled_config.caps_word = split_get_caps_word();
            if (transaction_rpc_send(RPC_SYNC_CONFIG, sizeof(oled_config), &oled_config)) {
